...
struct nodo {
	...
	int topt;
	vector<int> quedan;//Numero de mascarillas que lleva comrada cada universidad
	int completadas;
	int restantes;//Mascarillas totales que aún quedan por suministrar
};
...
int est_optimista(int precioT, int k, vector<int> precios,  vector<int> s, int restantes) {
	...
	int f = restantes;
	int optimo = precioT;
	while (restantes>0 && m < s.size()) {
		int ticket = min(f, s[m]);
		optimo += precios[m] * ticket;
		f -= ticket;
		m++;
	}
	if (f> 0)optimo = INT_MAX;
	return optimo;
}
...
bool resuelveCaso() {
	...
	int optimista = est_optimista(0, 0, precios, s, r);
	Y = { 0, 0, optimista, c,0 ,r};
	C.push(Y);
	...
	while (!C.empty() && C.top().topt < tiempomejor) {
		...
		for (int t = 0; t < N; t++) {//Recorro los hijos, que son universidades, para el caso de que le vende a una universidad.
			if (X.quedan[t] > 0) {//La universidad no ha satisfecho aún sus necesidades
				int ticket = min(X.quedan[t], s[X.k]);
				X.quedan[t] = Y.quedan[t] - ticket;
				X.tiempo = Y.tiempo + ticket * precios[X.k];
				X.restantes = Y.restantes - ticket;
				if (X.quedan[t] == 0) {
				...
				X.topt = est_optimista(X.tiempo, X.k, precios, s, X.restantes);
				if (X.topt < tiempomejor) {//Mejora la solución mejor encontrada hasta ese momento 
					if ((X.k == Z && X.completadas==N) || X.completadas == N) {//Si ya he comprado todos los objetos
