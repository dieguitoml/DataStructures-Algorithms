...
void resolver(string& entrada) {
	...
	for (int d = 1; d < n; d++) {//Recorrido por diagonales
		for (int i = 0; i < n - d; i++) {
			int j = i + d;	
			for (int k = i; k < j; k++) {
				if ((tabla[i][k][0] && tabla[k + 1][j][2]) || (tabla[i][k][1] && tabla[k + 1][j][2]) || (tabla[i][k][2] && tabla[k + 1][j][0]))tabla[i][k][0] = true;
				if ((tabla[i][k][0] && tabla[k + 1][j][0]) || (tabla[i][k][0] && tabla[k + 1][j][1]) || (tabla[i][k][1] && tabla[k + 1][j][1]))tabla[i][k][1] = true;
				if ((tabla[i][k][1] && tabla[k + 1][j][0]) || (tabla[i][k][2] && tabla[k + 1][j][1]) || (tabla[i][k][2] && tabla[k + 1][j][2]))tabla[i][k][2] = true;
			}
		}
	}
	if (tabla[n - 1][n - 1][0])cout << "SI" << '\n';
	else {
		cout << "NO" << '\n';
	}
}
