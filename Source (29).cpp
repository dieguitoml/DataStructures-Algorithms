...
pair<vector<int>, vector<int>> calculo_estimacion(vector<vector<int>> const& tiempos) {
    //Coste optimista
    vector<int> rapido(tiempos.size());
    vector<int> lento(tiempos.size());
    for (int i = 0; i < tiempos.size(); i++) {
        rapido[i] = tiempos[i][1];
        lento[i] = tiempos[i][1];
...
int resolver(vector<vector<int>>& tiempos) {
    ...
    Y.k = -1;
    pair < vector<int>, vector<int >> est = calculo_estimacion(tiempos);
    Y.tiempo_estimado = est.first[0];
    Y.tiempo = 0;
    Y.asignado = vector<bool>(tiempos.size(), false);
    Y.sol = vector<int>(tiempos.size());
    pila.push(Y);
    int coste_mejor = est.second[0];
    while (!pila.empty() && pila.top().tiempo_estimado <= coste_mejor) {
        ...
        for (int i = 0; i < tiempos.size(); i++) {
            if (!X.asignado[i]) {
                ...
                X.tiempo_estimado = X.tiempo +  est.first[X.k];
                if (X.tiempo_estimado <= coste_mejor) {
                    if (X.k == tiempos.size() - 1) {
                        ...
                        pila.push(X);
                        coste_mejor = min(coste_mejor, est.second[X.k]);
                    }
                }
                X.asignado[X.k] = false;
            }
        }
    }
    return coste_mejor;
