...
void resolver(vector<int> const& bizcocho) {
	...
	for (int d = 2; d < n; d++) {
		for (int i = 0; i < n - d; i++) {
			int j = i + d;
			if ((bizcocho[i] ==1 && bizcocho[j]==1)|| (bizcocho[i] == 2 && bizcocho[j] == 2))tabla[i][j] = max(tabla[i + 1][j - 1] + 1, max(tabla[i + 2][j], tabla[i][j - 2]));
			else {
				tabla[i][j] = max(tabla[i + 1][j - 1], max(tabla[i + 1][j], tabla[i][j - 2]));
			}
		}
	}
	cout << tabla[0][n - 1] << '\n';
