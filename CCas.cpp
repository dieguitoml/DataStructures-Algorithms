...
private:
    ...
    int count(Node* ptr, T const& lower, T const& upper) const;
};
...
template <typename T>
int SetTree<T>::count(Node* ptr, T const& lower, T const& upper) const{
    if (ptr == nullptr)return 0;
    else {
        if (ptr->elem < lower) {
            return count(ptr->right, lower, upper);
        }
        else if (ptr->elem > upper) {
            return count(ptr->left,lower, upper);
        }
        else if (ptr->elem == lower) {
            return count(ptr->right, lower, upper) + 1;
        }
        else if (ptr->elem == upper) {
            return count(ptr->left, lower, upper) + 1;
        }
        else {
            int iz = count(ptr->left,lower, upper);
            int dr = count(ptr->right, lower, upper);
            return iz + dr + 1;
        }
    }
}
...
int SetTree<T>::count_interval(T const& lower, T const& upper) const {
    return count(this->root_node, lower, upper);
}
